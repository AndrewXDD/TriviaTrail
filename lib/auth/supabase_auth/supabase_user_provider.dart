...
class TriviaTrailSupabaseUser extends BaseAuthUser {
  TriviaTrailSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> triviaTrailSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TriviaTrailSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
